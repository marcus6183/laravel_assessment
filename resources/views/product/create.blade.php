<x-layout>
    <div class="p-2">
        <div class="flex justify-between">
            <p class="text-3xl">Add New Product</p>
            <a class="px-3 py-2 bg-blue-500 rounded-md text-white" href="{{ route('product.home') }}">Back</a>
        </div>
        <form class="flex flex-col gap-4" action="{{ route('product.create') }}" method="POST">
            @csrf
            ...
            <div class="flex flex-col">
                ...
                <input class="border-2 border-neutral-400 p-2 rounded-md" type="float" name="price"
                    placeholder="99.90" value="{{ old('price') }}">
                @error('price')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-col">
                ...
                <textarea class="border-2 border-neutral-400 p-2 rounded-md" name="details" rows="10" placeholder="Detail">{{ old('details') }}</textarea>
                @error('details')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-col">
                ...
                <label>
                    <input type="radio" name="publish" value="yes"
                        {{ old('publish') === 'yes' || !old('publish') ? 'checked' : '' }}>
                    Yes
                </label>
                <label>
                    <input type="radio" name="publish" value="no" {{ old('publish') === 'no' ? 'checked' : '' }}>
                    No
                </label>
            </div>
